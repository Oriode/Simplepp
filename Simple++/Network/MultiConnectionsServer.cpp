#include "MultiConnectionsServer.h"


namespace Network {

	MultiConnectionsServer::MultiConnectionsServer(SockType sockType, IpFamily ipFamily) : Socket(sockType, ipFamily),
		mIsBinded(false)
	{
	...
	MultiConnectionsServer::~MultiConnectionsServer(){
		close();
	}



	bool MultiConnectionsServer::Listen(unsigned short port, int maxClients){
		return _listen(NULL, std::to_string(port).c_str(), maxClients);
	}


	bool MultiConnectionsServer::Listen(const std::string & ip, const std::string & service, int maxClients){
		return _listen(ip.c_str(), service.c_str(), maxClients);
	}

	bool MultiConnectionsServer::Listen(const std::string & ip, unsigned int port, int maxClients){
		return _listen(ip.c_str(), std::to_string(port).c_str(), maxClients);
	}

	bool MultiConnectionsServer::Listen(const AddrInfo & addrInfo, unsigned short port, int maxClients /*= 100*/){
	...
	bool MultiConnectionsServer::_listen(const char * ip, const char * service, int maxClients /*= 100*/){
	...
	bool MultiConnectionsServer::_tryListen(const ADDRINFO * addrResults, int maxClients){
	...
	bool MultiConnectionsServer::_tryListen(const Vector<const AddrInfo *> & addrInfoVector, int maxClients){
		bool result = false;
	...
	bool MultiConnectionsServer::_tryListen(const AddrInfo & addrInfo, int maxClients){
		if (addrInfo.getIpFamily() == IpFamily::Undefined){
			AddrInfo addrInfoV4(addrInfo);
			addrInfoV4.setIpFamily(IpFamily::IPv4);

			bool result1 = _tryListen(new Socket(addrInfoV4), maxClients);

			AddrInfo addrInfoV6(addrInfo);
			addrInfoV6.setIpFamily(IpFamily::IPv6);

			bool result2 = _tryListen(new Socket(addrInfoV6), maxClients);
			return result2 || result1;
		} else {
			return _tryListen(new Socket(addrInfo), maxClients);
		}
		return false;
	}


	bool MultiConnectionsServer::_tryListen(Socket * socket, int maxClients){
		if (this->mSocketVector.getSize() >= FD_SETSIZE) {
			warn("getaddrinfo returned more addresses than we could use.\n");
			return false;
		}
	...
	bool MultiConnectionsServer::close(){
		if (!this->mIsBinded) return false;
	...
	bool MultiConnectionsServer::Accept(Socket * clientSocket){
		if (getNumSocket() == 1)
			return Accept(0, clientSocket);

		Socket * selectedSocket = _select();
		if (selectedSocket)
	...
	bool MultiConnectionsServer::Accept(unsigned int socketIndex, Socket * clientSocket){
		return this->mSocketVector[socketIndex] -> Accept(clientSocket);
	}

	void MultiConnectionsServer::updateFdSet(){
		this->mFdSet.fd_count = (u_int) Math::min<Vector<Socket * >::Size>(this->mSocketVector.getSize(), 64);
	...
	size_t MultiConnectionsServer::getNumSocket() const{
		return this->mSocketVector.getSize();
	}

	Socket * MultiConnectionsServer::_select(){
	...
	bool MultiConnectionsServer::ReceiveFrom(char * buffer, int size, Address * addressFrom){
		Socket * selectedSocket = _select();
		if (selectedSocket)
